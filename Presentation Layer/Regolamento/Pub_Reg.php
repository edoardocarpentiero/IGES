<!--
	...
	Questa classe implementa l'interfaccia grafica per la pubblicazione di un regolamento
    esistente
...
?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            Regolamento
            <small>Pubblica Regolamento</small>
          ...
          <ol class="breadcrumb">
            ...
            <li class="active"><a href="">Pubblica Regolamento</a></li>
          </ol>
        </section>
            ...
            <!-- Left col -->
			 <div class="box box-default">
                <div class="box-header with-border">
                  <h3 class="box-title">Pubblica Regolamento <span id="nome"> </span></h3>
                ...
                <div class="box-body">
					<p>
						<form id="formCurriculum" name="formCurriculum" method="post">
						<table align="center" class="table table-bordered table-striped">
						<tr>
							<td style="text-align: center;">
									<section class="col-lg-12" style="text-align: center;">
									<div>
										<input type="radio" name="corso" value="Laurea" onclick="selezionaCurriculum()">
										<label>Laurea</label>
										<label>&nbsp&nbsp&nbsp</label>
										<input type="radio" name="corso" value="Magistrale" onclick="selezionaCurriculum()">
									...
									<section class="col-lg-12">
										...
										<section class="col-lg-4" id="selezioneCurriculum">
											<label>Seleziona Curriculum</label>
											<select class="form-control" style="text-align: center;" disabled name="curriculum" id="curriculum" onchange="mostraRegolamento(this.value)">
							                <option value="0" selected>Seleziona curriculum da pubblicare</option>
                                            </select>
							              </section>
									</section>					            	
			                </td>
...
<!-- DOPO LA SELEZIONE DEL CURRICULUM DOVRANNO COMPARIRE IN AUTOMATICO I DATI NELLE TABELLE -->
                    <p id="tabellaRegolamento">
...
<!-- CORPO BOX -->
		        	 ...
		        	 <section class="col-lg-12">
			        	<div class="box-footer" style="text-align: center;">
			        		<input type="button" class="btn btn-default btn-lg" value="Pubblica Regolamento" name="pubblica" id="pubblica" onclick="pubblica()" style="display:none">
			        	</div>
			        </section>
                </div><!-- /.box-body -->
			</div>
...
<script>
    ...
    function selezionaCurriculum(){
		var par=document.getElementById("tabellaRegolamento");
        par.innerHTML="";
        ...
        req.onreadystatechange=function(){
                if(req.status==200 && req.readyState==4){
                        ...
                        curr.disabled=false;
                        curr.innerHTML="<option value=\"0\" selected>Seleziona curriculum da pubblicare</option>";
                            for(i=0;i<curricula.length;i++)
                                    curr.innerHTML+='<option value="'+curricula[i]+'">'+curricula[i]+'</option>';
    ...
    function mostraRegolamento(valore){
    	if(valore!=0){
            ...
            req.onreadystatechange=function(){
                    if(req.status==200 && req.readyState==4){
                      	document.getElementById("tabellaRegolamento").innerHTML=req.responseText;
                        document.getElementById("pubblica").style.display="inline";
                    }
            }
        }
        else{
       	 	document.getElementById("tabellaRegolamento").innerHTML="";
            document.getElementById("pubblica").style.display="none";
        }
    }
    
    function pubblica(){
    	if(confirm("Pubblicare il regolamento?\nUna volta pubblicato il regolamento non è possibile più effettuare modifiche!")){
           req=new XMLHttpRequest();	
            var dati=new FormData();
            ...
            req.onreadystatechange=function(){
                    if(req.status==200 && req.readyState==4){
                      	if(parseInt(req.responseText)==0){
                            alert("Regolamento pubblicato con successo!");
                         	location.reload();
                         }
                  }
