...
<!--
	...
	Questa classe implementa l'interfaccia grafica che riguarda la modifica delle informazioni di un docente
...
<!--  TABELLA COMPLETA -->
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Modifica Stato Docenti</h3>
...
<!-- CORPO TABELLA -->
				<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <?php 
							...
							if(isset($_POST["submit"])){
								for($i=0;$i<count($docs);$i++){
									...
									if($nuovoStato!=$docs[$i]->getStato()){
										$res=$ges->modificaStatoDocente($docs[$i], $nuovoStato);
									}
								}
								$docs=$ges->ricercaDocente(1, "");
							}
							...
							for($i=0;$i<count($docs);$i++){
								...
								if($docs[$i]->getStato()==0){
									echo '<td><input type="radio" name="stato'.$i.'" value="1">Attivo</td>';
									echo '<td><input type="radio" name="stato'.$i.'" value="0" checked>Inattivo</td>';
								}
								else{
									echo '<td><input type="radio" name="stato'.$i.'" value="1" checked>Attivo</td>';
									echo '<td><input type="radio" name="stato'.$i.'" value="0">Inattivo</td>';
								}
								echo '</tr>';
							}
						?>
				  ...
				  <section>
				<!-- TASTO SALVATAGGIO MODIFICA STATO -->
                  <input type="submit" value="Salva Modifiche" class="btn btn-default btn-lg" name="submit">
                 </section>
				</form>
